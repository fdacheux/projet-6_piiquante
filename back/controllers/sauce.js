    ...
    try {
        ...
        if (!(await getSauce(req.params.id))) {
            ...
            try {
                switch (like) {
                    case 1:
                        const { modifiedCount: likesModified } = await addLike(
                            userId,
                            sauceId
                        )
                        if (likesModified > 0) {
                            ...
                            res.status(400).json({
                                message:
                                    'You are not authorized to add a like to this sauce',
                            })
                        }
                        break
                    case -1:
                        const { modifiedCount: dislikesModified } =
                        ...
                        break
                    case 0:
                        const { modifiedCount: dislikesLikesRemoved } =
                            await removeLikeDislike(userId, sauceId)
                        if (dislikesLikesRemoved > 0) {
                            res.status(200).json(
                                'Like/dislike successfully removed !'
                            )
                        } else {
                            res.status(400).json({
                                message:
                        ...
                        break
                    default:
                        throw new Error('Like format not recognized')
                }
            } catch (error) {
                res.status(400).json({ message: error.message })
            }
    ...
    try {
        ...
        if (!sauce) {
            ...
            try {
                ...
                res.status(200).json({
                    message: 'Sauce obect successfully deleted !',
                })
            } catch (error) {
                res.status(401).json({ message: error.message })
            }
